...
from cps_bot.cps.cps_api import extract_request_path, url_info
...
def load_menu_entry_paths(
    path: Path | str | None = None,
) -> dict[str, str]:
    """Đọc map menu_name → request_path (vd. mobile.html)."""
    file_path = Path(path or MENU_CATEGORY_MAP_PATH)
    if not file_path.is_file():
        return {}
    try:
        ...
        return {}
    entry_paths = data.get("entry_paths") or {}
    if not isinstance(entry_paths, dict):
        return {}
    return {str(k): str(v) for k, v in entry_paths.items()}
