...
_FEEDBACK_CONTEXT_MAX = 400
_feedback_context_cache: OrderedDict[str, dict[str, str]] = OrderedDict()


def _feedback_context_key(chat_id: str, message_id: str) -> str:
    return f"{chat_id}:{message_id}"


def cache_feedback_context(
    *,
    chat_id: str,
    message_id: str,
    user_question: str = "",
    ...
    if not chat_id or not message_id:
        return
    key = _feedback_context_key(chat_id, message_id)
    _feedback_context_cache[key] = {
    ...
    _feedback_context_cache.move_to_end(key)
    while len(_feedback_context_cache) > _FEEDBACK_CONTEXT_MAX:
        _feedback_context_cache.popitem(last=False)


def get_feedback_context(chat_id: str, message_id: str) -> dict[str, str]:
    if not chat_id or not message_id:
        return {}
    return dict(_feedback_context_cache.get(_feedback_context_key(chat_id, message_id), {}))
...
def record_message_feedback(
    *,
    ...
    emit_metric(
        ...
        user_question=(user_question or "")[:200],
        search_keywords=(search_keywords or "")[:120],
        product_id=str(product_id or ""),
        product_name=(product_name or "")[:120],
    )
    try:
        from .feedback_training import add_feedback_entry
        ...
        logger.warning("Feedback training queue failed: %s", exc)
